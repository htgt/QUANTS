...
process CUTADAPT {
    ...
    input:
    tuple val(meta), path(reads), val(cutadapt_option)
    ...
    def prefix         = cutadapt_option.suffix ? "${meta.id}${cutadapt_option.suffix}" : "${meta.id}"
    ...
    def untrimmed      = cutadapt_option.untrimmed ? untrimmed_cmd : ''

    """
    cutadapt \\
        --cores $task.cpus \\
        --json=${prefix}.cutadapt.json \\
        ${cutadapt_option.args} \\
        $trimmed \\
        $untrimmed \\
        $reads \\
        > ${prefix}.cutadapt.log
